...
public class BinarySearchTree {
    ...
    public Node _root;
    private Node _sentinel = new Node();


    public BinarySearchTree(Comparator comparator) {
        assert comparator != null : "nie określono komparatora";
        _comparator = comparator;
        _sentinel.setBlack(true);
        _sentinel.visited = true;
    }
    ...
    public void insert(Object x) {
        ...
        Node t = search(x);
        RestoreRBstructure(t);
    }

    protected Node insert(Object x, Node t) {
        if(t == null || t == _sentinel){
            t = new Node(x);
            t.left = _sentinel;
            t.right = _sentinel;
            t.setBlack(false);
    ...
    public void RestoreRBstructure(Node t) {
        //przywroc warunki drzewa RB
        if (t == root()) {
            t.setBlack(true);
        }
        //else {
            Node parent = findParent(t);
            if (parent != null && parent.isBlack() == false) {
                Node grandparent = findParent(parent);

                if (grandparent == null)
                    return;
                if (grandparent.right.isBlack() == false) {
                    grandparent.left.setBlack(true);
                    grandparent.right.setBlack(true);
                    //
                    ...
                    //
                        ...
                        RestoreRBstructure(grandparent);
                    return;
                }
                if (grandparent.right.isBlack() && t == parent.right) {
                    RotateLeft(parent);
                    RestoreRBstructure(parent);
                }
                if (grandparent.left.isBlack() && t == parent.left) {
                    RotateRight(parent);
                    RestoreRBstructure(parent);
                }
                if (grandparent.right.isBlack() && t == parent.left) {
                    RotateRight(grandparent);
                }
                if (grandparent.left.isBlack() && t == parent.right) {
    ...
    public void showTreePreorder(){
        ...
        this.preorder(root(), /*new LinkedList<Node>()*/ lista);
        for (Object n : lista){
    ...
    public List<Node> preorder(Node x, List<Node> argList){
        argList.add(x);
        if (x.left != null && x.left != _sentinel)
            preorder(x.left, argList);
        if (x.right != null && x.right != _sentinel)
    ...
    public void bfs()
    {
        ...
        q.add(root());
        System.out.println(root() + root().getColor());
        root().visited = true;
        while(!q.isEmpty())
        {
            ...
            {
                child.visited = true;
                System.out.println(child + child.getColor());
                q.add(child);
            }
        }
    ...
    public int height(Node aNode){
        int heightLeft = -1;
        int heightRight = -1;
        if(aNode.left != _sentinel)
            heightLeft = height(aNode.left);
        if(aNode.right != _sentinel)
    ...
    public int findMinimalNode(){
        if (this.isEmpty())
            ...
            return (int)minimalNode(root()).value;
        }
    }
    ...
    public int findMaximalNode(){
        if (this.isEmpty())
            ...
            return (int)maximalNode(root()).value;
        }
    }
    ...
    public int NodesWithTwoChildrenCount(Node node){
        if (this.isEmpty()){
            return 0;
        }
        else{
            return NodesTwoChildren(node);
        }
    }

    private int NodesTwoChildren(Node node){
        if (node.left != _sentinel && node.right != _sentinel) {
            return NodesTwoChildren(node.left) + NodesTwoChildren(node.right)+1;
        }
        else {
            return 0;
        }
    ...
    public int NodesWithOneChildrenCount(Node node){
        if (this.isEmpty()){
            return 0;
        }
        else{
            return NodesOneChildren(node);
        }
    }

    private int NodesOneChildren(Node node) {
        if (node.left != _sentinel && node.right != _sentinel) {
            return NodesOneChildren(node.left) + NodesOneChildren(node.right);
        }
        if (node.left == _sentinel && node.right != _sentinel){
            return NodesOneChildren(node.right) + 1;
        }
        if (node.right == _sentinel && node.left != _sentinel){
            return NodesOneChildren(node.left) + 1;
        }
        else {
            return 0;
        }
    ...
    public Node findParent(Node node){
        ...
        //boolean right = false;
        for (Node n : this.preorder(root(), new LinkedList<Node>())){
            if (n!=null && n.left == node || n.right == node ){
                parent = n;
                /*if (n.right == node){
                    right = true;
                }*/
    ...
    public void RotateLeft(Node n){
        ...
        if (parent == null){
            _root = x;
            x.left.setOppositeColor();
            RestoreRBstructure(x);
        }
    }
    ...
    public void RotateRight(Node n){
        ...
        if (parent == null){
            _root = x;
            x.right.setOppositeColor();
            RestoreRBstructure(x);
        }
    }

    public void UpdateNodesProperties(){
        for (Object n : preorder(root(), new LinkedList<Node>())){
            Node node = (Node)n;
            node.height = height(node);
            //nodesCount, nodesWithTwoChildrenCount, nodesWithOneChildrenCount;
            node.nodesCount = leavesCount(node);
            node.nodesWithTwoChildrenCount = NodesWithTwoChildrenCount(node);
            node.nodesWithOneChildrenCount = NodesOneChildren(node);
